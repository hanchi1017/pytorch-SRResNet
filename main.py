...
test_dir = "data/test"
crop_size = (128, 128)
cpt_dir = "model/checkpoint_vgg_relu22_2"
beta = 1

# debug mode
# train_dir = "data/minitest2"
# test_dir = "data/minitest2"
# crop_size = (128, 128)
# cpt_dir = "model/test"
# beta = 1
...
if not os.path.exists(cpt_dir):
    os.makedirs(cpt_dir)
evalResult = open("{}/eval_result.csv".format(cpt_dir), 'a')
trainResult = open("{}/train_result.csv".format(cpt_dir), 'a')
fieldnames = ['Epoch', 'Iteration','AvgPSNR', 'AvgCustomLoss', 'AvgContentLoss', 'lr', 'batch_num', 'seed']
evalWriter = csv.DictWriter(evalResult, fieldnames=fieldnames)
trainWriter = csv.DictWriter(trainResult, fieldnames=fieldnames)
evalWriter.writeheader()
trainWriter.writeheader()
SummWriter = SummaryWriter()
...
def main():
    ...
    mseLoss = nn.MSELoss()
    customLoss = CustomLoss(beta)

    print("===> Setting GPU")
    if cuda:
        model = model.cuda()
        customLoss = customLoss.cuda()
        if opt.vgg_loss:
    ...
    for epoch in range(opt.start_epoch, opt.nEpochs + 1):
        scheduler.step()
        train(training_data_loader, test_data_loader, optimizer, model, customLoss, mseLoss, epoch, opt.seed, cpt_dir)
...
def train(training_data_loader, test_data_loader, optimizer, model, customLoss, mseLoss, epoch, seed, cpt_dir):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        output = model(input)
        loss = customLoss(input, output, target, lr_mask, hr_mask)
        ...
        n_iter = (epoch-opt.start_epoch)*len(training_data_loader)+iteration
        if iteration%100 == 0:
            if opt.vgg_loss:
        ...
        if iteration%1 == 0:
            ...
            for name, param in model.named_parameters():
                SummWriter.add_histogram("seed_{}/{}".format(seed, name), param.grad.data.cpu(), n_iter)
            evaluate(test_data_loader, model, customLoss, mseLoss, opt.seed, epoch, iteration, optimizer.param_groups[0]["lr"], )
            evalWriter.writerow({'Epoch': epoch, 'Iteration': iteration, 'AvgCustomLoss': loss.data[0],
                                 'AvgContentLoss': content_loss.data[0], 'lr': optimizer.param_groups[0]["lr"], 'batch_num': len(test_data_loader),
                                 'seed': seed})
...
def evaluate(test_data_loader, model, CustomLoss, MSELoss, seed, epoch, iteration, lr, ):

    avg_psnr = 0
    avg_loss = 0
    avg_content_loss = 0
    for iterarion, batch in enumerate(test_data_loader, 1):
        input, target, lr_mask, hr_mask = Variable(batch[0], volatile=True), Variable(batch[1], volatile=True), \
                                          Variable(batch[2], volatile=True), Variable(batch[3], volatile=True)
        ...
        if opt.vgg_loss:
            ...
            content_target = netContent(target)
            content_loss = MSELoss(content_input, content_target)
            avg_content_loss += content_loss.data[0]

    avg_loss /= len(test_data_loader)
    avg_psnr /= len(test_data_loader)
    avg_content_loss /= len(test_data_loader)
    SummWriter.add_scalar("seed_{}/CustomLoss_mean".format(seed), avg_loss, epoch)
    SummWriter.add_scalar("seed_{}/psnr_mean".format(seed), avg_psnr, epoch)
    evalWriter.writerow({'Epoch':epoch, 'Iteration':iteration, 'AvgPSNR':avg_psnr, 'AvgCustomLoss':avg_loss,
                           'AvgContentLoss':avg_content_loss, 'lr':lr, 'batch_num':len(test_data_loader), 'seed':seed})
    if iteration%1000 == 0:
        print("seed {}: ===> Epoch: {}; Iteration: {}; Avg. CustomLoss: {:.4f}; Avg. PSNR: {:.4f} dB".format(seed,epoch,iteration,avg_loss,avg_psnr))   # len(test_data_loader) 为batch总数
